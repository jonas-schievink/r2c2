...
use self::types::{TypeRef, Type, AsType, IncompleteTypeRef, FnSig};
use utils::WriteStr;
...
impl<'a> Symbol<'a> {
    ...
    pub fn for_instance<'tcx>(inst: &Instance<'tcx>, tcx: TyCtxt<'_, 'tcx, 'tcx>) -> Self {
        Self {
            ...
            mangled: tcx.symbol_name(*inst).as_str().get(),
        }
    }
...
impl<'a, W: WriteStr> TranslationUnitBuilder<'a, W> {
    ...
    pub fn create(arena: &'a Arena, writer: W) -> io::Result<Self> {
        ...
        this.comment(&format!("generated by r2c2 version {}", env!("CARGO_PKG_VERSION")))?;
    ...
    pub fn separator(&mut self, name: &str) -> io::Result<()> {
        ...
        let text = format!("{}{}{}", " ".repeat(spaces / 2), name, " ".repeat((spaces + 1) / 2));
        self.comment(&text)?;
...
impl<'a, W: WriteStr> TranslationUnitBuilder<'a, W> {
    ...
    pub fn fwd_declare_struct(&mut self, sym: Symbol<'_>) -> io::Result<IncompleteTypeRef<'a>> {
        writeln!(self.writer, "struct {};  /* {} */", sym.mangled, sym.rust)?;
        let name = self.arena.alloc_str(sym.mangled());
        Ok(IncompleteTypeRef(self.arena.alloc(Type::FwdStruct { name })))
    }

    /// Emits a forward-declaration of a union `inst`.
    pub fn fwd_declare_union(&mut self, sym: Symbol<'_>) -> io::Result<IncompleteTypeRef<'a>> {
        writeln!(self.writer, "union {};  /* {} */", sym.mangled, sym.rust)?;
        let name = self.arena.alloc_str(sym.mangled());
        Ok(IncompleteTypeRef(self.arena.alloc(Type::FwdUnion { name })))
    }

    pub fn fwd_declare_function(&mut self, name: Symbol<'_>, proto: FnSig<'a>) -> io::Result<Function<'a>> {
    ...
    fn define_named_type<'f, F>(&mut self, kind: &str, sym: Symbol<'_>, fields: F) -> io::Result<TypeRef<'a>>
        where F: IntoIterator<Item = (&'f str, TypeRef<'a>)> {
        ...
        writeln!(self.writer, "{} {} {{  /* {} */", kind, sym.mangled, sym.rust)?;
    ...
    pub fn define_function<'b>(&'b mut self, arena: &'b Arena, name: Symbol<'_>, proto: FnSig<'a>) -> io::Result<FunctionBuilder<'b, W>> {
        Ok(FunctionBuilder::create(&mut self.writer, arena, name, proto)?)
    }
}
...
mod tests {
    ...
    fn redefinition() {
        ...
        test_tu(|f| {
            let bool = f.bool();
            f.define_struct(Symbol::test("my_struct"), vec![
                ("field1", bool),
            ])?;
            let msg = f.define_struct(Symbol::test("my_struct"), vec![
                ("field2", bool),
